# coding=utf-8
"""
Fine-tuning the GPT2 models for language modeling on a text file.
GPT2 model is fine-tuned using a causal language modeling (CLM) loss.
"""
...
from transformers import debugdict
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path: str, block_size=128):
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            with open(file_path, encoding="utf-8") as f:
                ...
                for line in lines:
                    line = line.replace('\n', '')
                    line = tokenizer.convert_tokens_to_ids(tokenizer.tokenize(line))
                    text += [bos_id] + line + [eos_id]
            tokenized_text = text
...
def train(args, train_dataset, model, tokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if global_step == 1: 
                for n, t in debugdict.items():
                    print(f'## {n}')
                    print(t, t.shape, t.stride(), t.storage_offset(), t.is_contiguous())
                    if t.grad is None:
                        continue
                    print(' -- grad --')
                    print(t.grad, t.grad.shape)
                exit()
...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--train_data_file", default='news_sample.txt', type=str, required=False, help="The input training data file (a text file)."
    )
    parser.add_argument(
        "--output_dir", default='extend_kogpt2_test',
        type=str,
        required=False,
        help="The output directory where the model predictions and checkpoints will be written.",
    )
    parser.add_argument(
        "--model_type", default='gpt2', type=str, required=False, help="The model architecture to be trained or fine-tuned.",
    )

    # Other parameters
    parser.add_argument(
        "--eval_data_file",
        default='news_sample.txt',
        type=str,
